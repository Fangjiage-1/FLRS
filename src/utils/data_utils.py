...
def load_dataset_split(path: str, dataset_cache_dir=None):
    ...
    try:
        ...
        try:
            ...
            pass  # fall through to snapshot_download below
    ...
    if ds is None or _looks_like_save_to_disk_arrow(ds):
        from huggingface_hub import snapshot_download
        if ds is None:
            log_for_0(
                f"Dataset at {path!r} could not be loaded directly; "
                f"falling back to snapshot_download + load_from_disk."
            )
        else:
            log_for_0(
